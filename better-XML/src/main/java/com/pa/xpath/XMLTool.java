...
//  private ListModel<LibrarySection> librarySectionModel = new DefaultListModel<>();
	...
	public XMLTool() {
		...
		lstXPathSections.setModel(new DefaultListModel());
...
//  }
	...
	private void buildXPathLibrarySectionsList() {
		((DefaultListModel) lstXPathSections.getModel()).removeAllElements();
		List<XPathLibrarySection> sections = DataService.getLibrarySections();
		for (XPathLibrarySection section : sections) {
			((DefaultListModel<XPathLibrarySection>) lstXPathSections.getModel()).addElement(section);
		}
	}
...
//      DefaultMutableTreeNode foundNode = searchChildren(fullStringSearch, treeModel, startTreeNode, txtTreeSearch.getText().toLowerCase());
  ...
  private void btnDeleteSectionActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteSectionActionPerformed
		XPathLibrarySection section = (XPathLibrarySection) lstXPathSections.getSelectedValue();
  ...
  private void lstXPathSectionsValueChanged(javax.swing.event.ListSelectionEvent evt) {//GEN-FIRST:event_lstXPathSectionsValueChanged
		selectedLibrarySection = (XPathLibrarySection) lstXPathSections.getSelectedValue();
		...
		clearSelectedDataNotSection();
		buildSaveButtonText();
  }//GEN-LAST:event_lstXPathSectionsValueChanged
  ...
  private void btnSaveEditedXPathItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSaveEditedXPathItemActionPerformed
		...
		if (sectionExists) {
			...
			if (!xpathName.trim().equals("")) {
				...
				DataService.createXPathLibrarySection(sectionName);
			}
			buildXPathLibrarySectionsList();
			lstXPathSections.setSelectedValue(section, true);
		}
...
//            return searchChildren(treeModel, treeNode, searchString);
	...
	private boolean sectionExistsInModel(String sectioName) {
		Enumeration<XPathLibrarySection> sections = ((DefaultListModel<XPathLibrarySection>) lstXPathSections.getModel()).elements();
		DefaultListModel x;
